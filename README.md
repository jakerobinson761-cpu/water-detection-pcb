...
Prior to discovering the issue was the DC Jack, which if unfixed the entire project wouldn't work as there wouldn't be a constant power supply, I suspected it was the PMOS that powers the green LED when water isn't on. This was the third attempt and my third PMOS when I was fixing it, so two PMOSs where destroyed in the process of making this project. The reason why I suspected it was the PMOS is because the PMOS would shift when any force was applied to it, and the PMOS is integral to supplying voltage to the green LED when water isn't touching the connector.
