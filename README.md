...
### Big Idea
Whenever water touches a connector attached to this PCB it will sound a buzzer, turn on a red LED, and send a notification to the user's phone that water touched the connector. Whenever the connector isn't being touched by water a green LED will be turned on using backwards logic to how the red LED turns on (the red LED uses a N-channel MOSFET, but the green LED uses a P-Channel MOSFET). But how? Water touches the water-probe connector (J2) and this causes conductivity between pin 1 and pin 2 of this connector. This conductivity allows the VCC to be drawn into the gates of three MOSFETs (P and N channel). The reason is simple: VCC is constantly being sent to pin 1 of the water-probe connector, and pin 2 is extremely close to pin 1. The water acts as a wire to allow both pins to connect. Explanations of all of how MOSFETs work and how the N-Channel and P-Channel MOSFET work in this project are below.
